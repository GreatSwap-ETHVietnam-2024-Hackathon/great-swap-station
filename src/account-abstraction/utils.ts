...
export function hexlifyUserOp(userOp: UserOperation): UserOperation {
  let result: any = {}
  Object.entries(userOp).forEach(([key, value]) => {
    if (value === "0x" || value === undefined || value === null) result[key] = "0x"
    else result[key] = ethers.BigNumber.from(value).toHexString()
  })
